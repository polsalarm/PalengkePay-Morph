...
import { ArrowDownToLine, Loader2, AlertTriangle, ShieldCheck } from 'lucide-react';
...
export function CustomerCashin() {
  ...
  const launch = () => {
    if (!transakConfigured) {
      showToast('Cash-in not configured — set VITE_TRANSAK_API_KEY', 'error');
      return;
    }
    setBusy(true);
    openTransak({
      ...
      onSuccess: () => { showToast('Order placed! ETH arrives shortly.', 'success'); refetch(); },
  ...
  return (
    <div className="max-w-md mx-auto space-y-4 animate-page-in">
      <div className="relative rounded-3xl overflow-hidden p-5" style={{ backgroundColor: '#00284B' }}>
        ...
        <p className="text-sm" style={{ color: 'rgba(255,255,255,0.55)' }}>Buy ETH with PHP via Transak. Funds land in your wallet.</p>
      ...
      >
        ...
        {busy ? 'Opening Transak…' : 'Buy ETH with PHP'}
      </button>
